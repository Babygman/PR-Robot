"""Test Full RBAC (Correction 2026-09-10) — 9 เมนูตาม Matrix Admin/FA/Approve/PR/AR/ALL ที่
ผู้ใช้ยืนยัน ครอบคลุม:
...
- Ownership Scoping: ติ๊ก can_view_pr/can_view_ar อย่างเดียว เห็นเฉพาะ PR/AR ของตัวเอง —
  can_view_all ขยายให้เห็นทั้งหมด แต่ไม่ได้แปลว่าสร้างใหม่ได้ (ยังต้องมี can_view_pr/ar)
- PR ไม่มีผู้อนุมัติในระบบ (Scope Revision Phase 9) — บังคับ Owner-only ทุก Endpoint จริง
  (get/update/revise/pdf/history) เว้นแต่ Admin/can_view_all
...
- Log (/audit-logs): Admin/FA/can_view_all เท่านั้น รวมกิจกรรม PR+AR
"""
...
def test_can_view_all_alone_cannot_create_pr(client: TestClient, db_session: Session):
    """can_view_all คือสิทธิ์ดูภาพรวมอย่างเดียว ไม่ได้แปลว่าสร้าง PR แทนคนอื่นได้"""
    viewer = _make_user(
        db_session,
        name="Viewer PR",
    ...
    # แต่เข้าเมนู (List) ได้ปกติเพราะ can_view_all ขยายสิทธิ์ดูให้
...
def test_pr_can_view_all_sees_everyone(client: TestClient, db_session: Session):
...
def test_can_view_all_alone_cannot_create_ar(client: TestClient, db_session: Session):
    viewer = _make_user(
        db_session,
        name="Viewer AR",
...
def test_ar_can_view_all_sees_everyone_in_list(client: TestClient, db_session: Session):
...
# ───────────────────────── Log (/audit-logs) ─────────────────────────
def test_audit_logs_require_login(client: TestClient):
    assert client.get("/audit-logs").status_code == 401


def test_audit_logs_forbidden_for_plain_user(client: TestClient, db_session: Session):
    user = _make_user(
        db_session,
        name="No Log",
...
def test_audit_logs_allowed_for_admin_fa_and_can_view_all(
    client: TestClient, admin_user: User, db_session: Session
):
    fa = _make_user(db_session, name="FA Log", email="falog@example.com", is_fa=True)
    viewer = _make_user(
        db_session,
        name="ViewAll Log",
        email="viewalllog@example.com",
        can_view_pr=False,
        can_view_ar=False,
        can_view_all=True,
    )
...
def test_audit_logs_combine_pr_and_ar_with_display_fields(
    client: TestClient, admin_user: User, db_session: Session
):
    ...
    pr_log = next(log for log in logs if log["doc_type"] == "pr" and log["doc_id"] == pr_id)
    assert pr_log["action"] == "pr.created"
    assert pr_log["doc_no_display"] is not None

    ar_log = next(log for log in logs if log["doc_type"] == "ar" and log["doc_id"] == ar_id)
    assert ar_log["action"] == "ar.created"
    assert ar_log["doc_subject"] == _sample_ar_body()["subject"]


def test_audit_logs_filter_by_doc_type(client: TestClient, admin_user: User, db_session: Session):
    ...
    _login_as(client, admin_user.email, "adminpass123")
    pr_only = client.get("/audit-logs?doc_type=pr").json()
    assert all(log["doc_type"] == "pr" for log in pr_only)
    ar_only = client.get("/audit-logs?doc_type=ar").json()
    assert all(log["doc_type"] == "ar" for log in ar_only)
